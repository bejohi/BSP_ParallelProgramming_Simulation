...
void leastsquares(long h0, long h1, double *t, double *g, double *l){
    ...
    long nh= h1-h0+1;
    long sumh= (h1*(h1+1) - (h0-1)*h0)/2;  
    long sumhh= (h1*(h1+1)*(2*h1+1) - (h0-1)*h0*(2*h0-1))/6;
...
void bspbench(){
        ...
        if (s==0){
            ...
            if (mintime>0.0){
                ...
                for(long s1=0; s1<p; s1++)
                    r += nflops/Time[s1];
                r /= p; 
                printf("n= %5ld min= %7.3lf max= %7.3lf av= %7.3lf Mflop/s ",
    ...
    for (long h=0; h<=MAXH; h++){
        ...
        for (long iter=0; iter<NITERS; iter++){
            for (long i=0; i<h; i++)
                bsp_put(destproc[i],&src[i],dest,destindex[i]*sizeof(double),sizeof(double));
            bsp_sync(); 
        }
        double time1= bsp_time();
        double time= time1-time0;
    ...
    if (s==0){
        double g0, l0, g, l; 
        printf("size of double = %ld bytes\n",(long)sizeof(double)); //may be useful info
