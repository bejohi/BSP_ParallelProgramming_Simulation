...
void cannonMatrixMult() {
    ...
    for (int iteration = 0; iteration < s; iteration++) {
        ...
        if (DEEP_DEBUG)
            printf("iteration %d start for processorId=%d, downId=%d, rightId=%d\n", iteration, processorId, downId,
                   rightId);
    ...
    if (processorId == 0) {
        ...
        bsp_get(iProcessor, pointerC, ((iToCheck % nrows) * nrows + jToCheck) * sizeof(double), &result,
                sizeof(double));
